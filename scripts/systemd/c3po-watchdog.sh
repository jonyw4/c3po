...
#
# NOTE: This is a TEMPLATE. The rendered version (c3po-watchdog.local.sh)
# is generated by render-files.ts and should NOT be committed to Git.
...
# Use XDG_RUNTIME_DIR (user-private, tmpfs) with fallback
STATE_DIR="${XDG_RUNTIME_DIR:-${HOME}/.local/state/c3po}"
mkdir -p "$STATE_DIR"
STATE_FILE="${STATE_DIR}/c3po-watchdog-alerted"

ALERT_NUMBER="+55YYYYYYYYYYY"
...
send_alert() {
    ...
    if command -v openclaw >/dev/null 2>&1; then
        openclaw send --channel whatsapp --to "$ALERT_NUMBER" --message "$msg" 2>/dev/null || true
    fi
